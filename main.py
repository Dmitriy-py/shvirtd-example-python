...
log_table_name = os.environ.get('LOG_TABLE_NAME', 'requests')
...
async def lifespan(app: FastAPI):
    ...
    try:
        with get_db_connection() as db:
            ...
            CREATE TABLE IF NOT EXISTS {db_name}.{log_table_name} (
...
def index(request: Request, ip_address: Optional[str] = Depends(get_client_ip)):
    ...
    try:
        with get_db_connection() as db:
            cursor = db.cursor()
            query = f"INSERT INTO {log_table_name} (request_date, request_ip) VALUES (%s, %s)"
...
def get_requests():
    ...
    try:
        with get_db_connection() as db:
            cursor = db.cursor()
            query = f"SELECT id, request_date, request_ip FROM {log_table_name} ORDER BY id DESC LIMIT 50"
